    ...
    public class MeshGeneratorUtils
    {
        ...
        public static void MakeTetsForwardFacing(TetMesh4D.Tet4D[] tets, TetMesh4D.VertexData[] meshVertices)
        {
            for (int idx = 0; idx < tets.Length; ++idx)
            {
                ...
                Vector3 S4DGE = (meshVertices[p2Idx].position - p0Pos);
                Vector3 v3 = (meshVertices[p3Idx].position - p0Pos);

                int volumeSign = Math.Sign(Vector3.Dot(v1, Vector3.Cross(S4DGE, v3)));

                if (volumeSign < 0)
                {
                    Debug.Log("volume less than zero: " + volumeSign + string.Format("\nindices: {0}, {1}, {2}, {3}", p0Idx, p1Idx, p2Idx, p3Idx));
                }
